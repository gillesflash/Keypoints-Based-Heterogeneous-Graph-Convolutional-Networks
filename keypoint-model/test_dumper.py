...
keypoints_classes_ids2names = {0: 'dumper_le', 1:'dumper_lf', 2:'dumper_re', 3:'dumper_rf'}
...
KEYPOINTS_FOLDER_TEST = './dataset0/DT_dumper/train'
...
model.load_state_dict(torch.load('./model/weights/keypointsrcnn_weights_dumper0.pth'))
...
for images, targets in iterator:
    ...
    """==========================================================================="""
    #             angle calculation
    midx_end = (keypoints[0][0][0] + keypoints[0][2][0])/2
    midy_end = (keypoints[0][0][1] + keypoints[0][2][1])/2
    midx_f = (keypoints[0][1][0] + keypoints[0][3][0])/2
    midy_f = (keypoints[0][1][1] + keypoints[0][3][1])/2
    dx = midx_f - midx_end
    dy = midy_f - midy_end
    angle = math.atan2(dy,dx)
    oritation = round((angle-ang_0)/math.pi*180, 2)
    # print('angle is : ', ang_0)
    print('dumper angle is: ', oritation)
    # # print('dy is: ', dy)
    #
    ...
    keypoints_result = pd.DataFrame(keypoints_out)
    keypoints_result.to_csv('DT_dumper.txt', sep='\t', index=False)
