...
keypoints_classes_ids2names = {0: 'dozer_end', 1:'dozer_l', 2:'dozer_r'}
...
KEYPOINTS_FOLDER_TEST = './dataset0/DT_dozer/train'
...
model.load_state_dict(torch.load('./model/weights/keypointsrcnn_weights_dozer0.pth'))
...
for images, targets in iterator:
    ...
    midx_end = keypoints[0][0][0]
    midy_end = keypoints[0][0][1]
    midx_f = (keypoints[0][1][0] + keypoints[0][2][0])/2
    midy_f = (keypoints[0][1][1] + keypoints[0][2][1])/2
    dx = midx_f - midx_end
    dy = midy_f - midy_end
    angle = math.atan2(dy,dx)
    oritation = round((angle-ang_0)/math.pi*180-ang_0, 2)
    ...
    keypoints_result.to_csv('DT_dozer.txt', sep='\t', index=False)
